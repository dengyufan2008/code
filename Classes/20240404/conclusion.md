...
- LGV Lemma: 对于两条路径 $i \rightarrow l$ 与 $j \rightarrow k$, 满足 $i$ 在 $j$ 的左侧, $k$ 在 $l$ 的左侧, 若必然有这两条路径交叉, 那么对于 $|L| = |R| = n$ 的两个点集之间对应有路径且不交的方案数为 矩阵 $A_{i, j} = (L$ 中 $i$ 号元素到 $R$ 中 $j$ 号元素的路径条数$)$ 的行列式.
