...
所有区间都包含点集 $S$ 中之一, 且不存在 $T$ 满足 $|T| \le |S|, T \neq S$ 使得所有区间都包含点集 $T$, 可以转化为对于 $S$ 中每个元素 $x$, 尝试将它修改为前驱和后继 (一般为 $x-1$ 和 $x+1$), 判断新的 $S'$ 能否覆盖所有区间. 当 $|S|$ 大小只有 $O(1)$ 时, 可以手动分类讨论并进一步得出更详细的判定条件.
